...
export class ActiveItems extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ActiveItems entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ActiveItems must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ActiveItems", id.toString(), this);
    }
  }

  static load(id: string): ActiveItems | null {
    return changetype<ActiveItems | null>(store.get("ActiveItems", id));
  }
