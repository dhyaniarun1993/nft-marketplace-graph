...
import { ActiveItems, ItemBought, ItemCanceled, ItemListed } from "../generated/schema"

export function handleitemBought(event: itemBoughtEvent): void {
  let itemBought = ItemBought.load(getIdFromEventParams(event.params.tokenId, event.params.nftAddress))
  let activeItem = ActiveItems.load(getIdFromEventParams(event.params.tokenId, event.params.nftAddress))
  ...
  // item should be part of active item
...
export function handleitemListed(event: itemListedEvent): void {
  ...
  if (!activeItem) {
    activeItem = new ActiveItems(getIdFromEventParams(event.params.tokenId, event.params.nftAddress))
  }
